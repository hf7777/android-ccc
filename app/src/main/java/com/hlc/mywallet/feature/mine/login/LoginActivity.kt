...
class LoginActivity : BaseVbActivity<ActivityLoginBinding>() {
    ...
    override fun initView() {
        ...
        binding.tvForgotPassword.onClick {
            Router.navigation(Routes.SET_PASSWORD)
                .with(Constants.RouterKeys.SHOW_SIGN, true)
                .navigation(this)
        }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.savedPhone.collect { phone ->
                    if (phone.isNotEmpty() && binding.etPhoneNumber.text.isNullOrEmpty()) {
                        binding.etPhoneNumber.setText(phone)
                        binding.etPhoneNumber.setSelection(phone.length)
                    }
                }
            }
        }
